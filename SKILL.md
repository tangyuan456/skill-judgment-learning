...
description: "数据库性能测试报告生成技能。支持两种数据源：①本地数据/数据文件（.log/.xlsx/.json/.csv）直接读取；②task_id/plan_id/report_id 从 yunyu_test_results PostgreSQL 数据库查询。支持四种报告类型：single（单次测试报告）/ comparison（性能对比报告）/ iteration（项目迭代报告）/ custom（客制化/专项报告）。输出 md + docx + html 三格式。触发词：测试报告、性能报告、对比报告、迭代报告、客制化报告、task_id 报告、sysbench 报告、benchmarksql 报告。"
---
...
本 Skill 是「TDSQL-B 白皮书 Skill」的通用化升级版，统一处理两类输入源、四种报告类型，输出 md/docx/html 三格式报告：

| 数据源 | 触发条件 | 处理方式 |
|--------|---------|---------|
| **本地数据/数据文件** | 用户提供 `.log` / `.xlsx` / `.json` / `.csv` 路径，或直接粘贴数据 | 解析为标准 records 结构，跳过数据库查询 |
| **task_id / plan_id / report_id** | 用户输入含任务标识符 | 经 LLM 拆解 → 生成 SQL → 查询 `yunyu_test_results` |

| 报告类型 | report_type | 触发关键词 | 输出特征 |
|---------|-------------|-----------|---------|
| **单次测试报告** | `single` | 默认（无特殊关键词） | 1 份测试数据 + 5 场景指标分析 |
| **性能对比报告** | `comparison` | 对比 / 比较 / vs / 差异 / 哪个更好 | ≥ 2 份测试数据横向对比（产品 / 版本 / 配置） |
| **项目迭代报告** | `iteration` | 迭代 / 版本演进 / 历史趋势 | ≥ 3 个版本/时间点的演进趋势 |
| **客制化报告** | `custom` | 客制化 / 专项 / 定制 / 深度分析 | 用户指定分析维度的深度报告 |

> ⚠️ **强制约束**：每次调用本 Skill 时，必须完整遵守本文件及所有 reference 文件中的规范，不得跳过、简化或选择性执行。

---

## 与上游 Skill 的差异

| 维度 | TDSQL-B 白皮书 Skill | dify 报告智能生成 yml | 本 Skill |
|------|---------------------|----------------------|---------|
| 数据源 | 仅本地 .log | 仅 PG 数据库 | **本地 / 数据库 二选一** |
| 报告类型 | 仅产品对比白皮书 | 4 类 | **4 类** |
| 数据筛选 | 无 | LLM → SQL → 模糊匹配 | **保留 LLM 拆解 + 增加场景关键词中英映射** |
| 三格式 | md + docx + html | md + html | **md + docx + html** |
| 洞察分级 | L1/L2/L3 强制标注 | 无 | **保留 L1/L2/L3** |
| 公平性核查 | 强制（数据集/并发/时长一致） | 无 | **保留** |

---

## 核心原则（强制）

1. **数据源识别先行**：`scripts/detect_input.py` 自动识别用户输入是「本地路径」「数据库标识」还是「自然语言+标识符」。
2. **意图解析**：自然语言 → JSON（按 `references/意图解析规范.md` 提取 `report_type / dimension_filters / task_filters`）。
3. **数据铁律**：所有报告数据必须来自 ①本地原始文件 或 ②`yunyu_test_results` 表。**禁止 AI 推断、估算、硬编码数据。**
4. **场景分离**：分析章节按 5 个 sysbench 场景（point_select / read_only / write_only / read_write / update_index）独立输出，禁止合并均值。
5. **门控确认**：3 个门控（提案 / 数据 / 报告）必须向用户展示规定内容并获得明确 ✅。
6. **洞察分级**：
...
阶段2: 数据接入
        ...
        └─ task_id：LLM→SQL→PostgreSQL→标准 records.json
        ↓
        🚧 门控②（数据确认）
        ↓
...
| 含 PostgreSQL 表数据片段（粘贴的 JSON） | `local_data` | `{"test_name":"...","results":{...}}` |
| 含 8~64 位字母数字 + 下划线组合（含 `_tdsql_` 等模式） | `task_id` | `878fd4d4_tdsql_intel_x86` |
| 含 "task_id" / "plan_id" / "report_id" 关键词 + 标识符 | `task_id` | `task_id 为 dd270144` |
| 上述均不匹配 | `keyword_only` | `集中式只读场景` |

### Step 1.2 报告类型识别（同 dify yml 规则）
...
def load_records(intent_json) -> StandardRecords:
    ...
    根据 intent_json.data_source_type 派发到不同适配器：
      ...
      - task_id:      构建 SQL → 查询 PostgreSQL → 转标准结构
      - keyword_only: 仅根据关键词查询 PG（最宽泛）
...
- `.json`：要求最小字段 `{product, scenario, threads, tps, qps, p95_ms}`
- `.csv`：表头匹配同 .json

### Step 2.3 数据库查询

- **默认走 HTTP API**（已内置连接，无需额外配置）：
  - 入口：`POST http://yunyu.woa.com/test/api/alarm/sql_query/`
  - 认证：`YUNYU-SYS-ID: check` + `YUNYU-TOKEN: <内置>` + body `p_id=1`
  - 脚本：`scripts/db_query_http.py`
  - 配置：`config/yunyu_api.yaml`（内置 URL/Token/PID）
- **可选切到 PostgreSQL 直连**：设 `export USE_DIRECT_PG=1` 并填写 `config/db.yaml`
- 数据库表：`yunyu_test_results`
- 关键字段：`task_id` / `plan_id` / `report_id` / `dimension`(jsonb) / `results`(jsonb) / `results_key`(varchar)
- SQL 生成模板：`templates/query_templates.sql`
- **UUID 格式 task_id 校验**：形如 `75ff5153-e6db-4a6a-83ac-1a7828b1869b` 的 UUID 不在 yunyu_test_results 表中（属于 MrThree），用 `scripts/probe_mrthree.py` 先做存在性校验

⚠️ **严禁双引号**（API 限制）：所有 SQL 字段名/表名不加 `"`，仅字符串值加 `'`。

### Step 2.4 标准化为 records
...
```json
{
  "meta": {
    ...
    "source_info": {
      "type": "task_id",
      "value": "878fd4d4_tdsql_intel_x86",
      "rows_fetched": 60
    }
  },
  "records": [...]
...
| comparison | 同 TDSQL-B 白皮书 Skill 的 P0+P1（峰值×5 + 并发-QPS×5 + 并发-P95×5 + 雷达 + 天梯 + 单产品×N） |
...
**禁止**：用 pandoc 等工具直接 md → docx/html。
...
### 禁止事项
- 跳过 Reference 直接执行
- 看到 task_id 就忽略场景关键词（必须共存提取）
- 把 OR 关系（"和/或"连接不同场景）误识别为 AND
- 把中文业务术语原样写入 SQL（必须映射为英文字段）
...
pip install "openpyxl>=3.1.0" "matplotlib>=3.7.0" "numpy>=1.24.0" \
            ...
            "markdown>=3.5.0" "psycopg2-binary>=2.9.0" "pyyaml>=6.0"
```
...
| `scripts/data_source_adapter.py` | 统一数据接入（local/db/http） | 阶段2 |
| `scripts/db_query.py` | PostgreSQL 查询（直连，可选） | 阶段2 |
| `scripts/db_query_http.py` | **YunYu HTTP API 查询（默认）** | 阶段2 |
| `scripts/probe_mrthree.py` | UUID 形式 task_id 存在性校验 | 阶段2 |
...
| `templates/query_templates.sql` | SQL 查询模板 | 阶段2 |
